...
//		handlers.keyPress.ForEach(h => keyPress.Add(h));
		AddToHandlerList(load, handlers.load);
		AddToHandlerList(unload, handlers.unload);
		AddToHandlerList(enterFrame, handlers.enterFrame);
		AddToHandlerList(update, handlers.update);
		AddToHandlerList(render, handlers.render);
		AddToHandlerList(press, handlers.press);
		AddToHandlerList(release, handlers.release);
		AddToHandlerList(rollOver, handlers.rollOver);
		AddToHandlerList(rollOut, handlers.rollOut);
		AddToHandlerList(keyPress, handlers.keyPress);
		// EBG END - GC OPTIM
	}
	
	// EBG START - GC OPTIM
	private void AddToHandlerList(List<Action<Button>> handlerList, List<Action<Button>> handlersToAdd)
	{
		if (handlerList == null) return;
		if (handlersToAdd == null || handlersToAdd.Count == 0) return;

		for (int i = 0; i < handlersToAdd.Count; ++i)
		{
			handlerList.Add (handlersToAdd[i]);
		}
	}

	private void AddToHandlerList(List<Action<Button, int>> handlerList, List<Action<Button, int>> handlersToAdd)
	{
		if (handlerList == null) return;
		if (handlersToAdd == null || handlersToAdd.Count == 0) return;
