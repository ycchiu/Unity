...
public partial class Movie : IObject
{
	...
	public override void Update(Matrix m, ColorTransform c)
	{
		...
		m_invokeUpdateDelegate = colorTransformChanged || matrixChanged;
	...
	public override int RefreshDepth(int startingDepth = 0)
	{
		int curDepth = startingDepth;
		for (int dlDepth = 0; dlDepth < m_data.depths; ++dlDepth)
		{
			Object obj = m_displayList[dlDepth];
			if (obj != null)
			{
				curDepth = obj.RefreshDepth(curDepth);
			}
		}
		return curDepth;
	}
	...
	{
		for (int i = 0; i < data.depths; ++i)
		{
			if (m_displayList[i].type == Type.BITMAP)
			{
				return m_displayList[i] as Bitmap;
			}
		}
		return null;
	}

	public override int numChildren { get { return data.depths; } }
	public override Object[] children { get { return m_displayList; } }
	// EBG END
