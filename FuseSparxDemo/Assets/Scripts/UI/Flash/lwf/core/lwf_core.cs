...
{
	public static Action Wrap(System.Object b, System.Action cb)
	{
		System.Action action = delegate() { if (b != null) { cb(); } };
		return action;
	}

	public static Action<T> Wrap<T>(System.Object b, System.Action<T> cb)
	{
		System.Action<T> action = delegate(T obj) { if (b != null) { cb(obj); } };
		return action;
	}

	public static Action<T,U> Wrap<T,U>(System.Object b, System.Action<T,U> cb)
	{
		System.Action<T,U> action = delegate(T obj1, U obj2) { if (b != null) { cb(obj1, obj2); } };
		return action;
	}
}
// EBG END
...
public partial class LWF
{
	...
	{
		get { return m_hierarchyChanged; }
		set { m_hierarchyChanged = value; }
	}

	public Action<LWF> onHierarchyUpdatedDelegate;
	// EBG END
		...
		Matrix matrix = null, ColorTransform colorTransform = null)
	{
		...
		if (m_execDisabled && tweens == null) {
			...
			// EBG START - GC OPTIM
				...
				{
					for (int i = 0; i < m_execHandlers.Count; ++i)
					{
						m_execHandlers[i](this);
					}
				}
			// EBG END
		...
		{
			m_rootMovie.RefreshDepth();
			m_hierarchyChanged = false;
			if (onHierarchyUpdatedDelegate != null)
			{
				onHierarchyUpdatedDelegate.Invoke (this);
			}
		}
		// EBG END
...
//				movie = movie.SearchMovieInstance(name);
			...
			{
				movie = movie.SearchMovieInstance (kvp.Key[i]);
			// EBG END
				if (movie == null) {
					available = false;
					break;
...
//			m_movieCommands.Remove(key);
		for (int i = 0; i < deletes.Count; ++i)
			m_movieCommands.Remove(deletes[i]);
		// EBG END
	}
