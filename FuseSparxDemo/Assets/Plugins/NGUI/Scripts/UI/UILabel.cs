...
public class UILabel : UIWidget
{
	...
	// EBG START - FontSelector support
	[HideInInspector][SerializeField] FontSelector mSelectorFont;
	[HideInInspector][SerializeField] CustomFontBase mCustomFont;
	// EBG END
#if !UNITY_3_5
	...
	public override Material material
	{
		get
		{
			...
			// EBG START
			if (mSelectorFont != null) return mSelectorFont.material;
			if (mCustomFont != null) return mCustomFont.material;
			// EBG END
			return null;
		}
		set
	...
	public UIFont bitmapFont
	{
		...
		{
			// EBG START
			if (mSelectorFont != null && mSelectorFont.selectedFont != null) return mSelectorFont.selectedFont;
			if (mCustomFont != null && mCustomFont.isBitmap) return mCustomFont.bitmapFont;
			// EBG END
			return mFont;
		}
		set
	...
	public Font trueTypeFont
	{
		get
		{
			...
			// EBG START
			else if (mSelectorFont != null && mSelectorFont.selectedTrueTypeFont != null) return mSelectorFont.selectedTrueTypeFont;
			else if (mCustomFont != null && mCustomFont.isTrueType) return mCustomFont.trueTypeFont;
			else
			// EBG END
	...
	// EBG START
	/// <summary>
	/// Sets the FontSelector used by this label.
	/// </summary>
	
	public FontSelector selectorFont
	{
		get
		{
			return mSelectorFont;
		}
		set
		{
			if (mSelectorFont != value)
			{
#if DYNAMIC_FONT
				SetActiveFont (null);
				RemoveFromPanel();
				...
				if (value != null)
				{
					SetActiveFont (value.selectedTrueTypeFont);
				}
				ProcessAndRequest();
				if (mActiveTTF != null)
					base.MarkAsChanged();
	...
	public CustomFontBase customFont
	{
		get
		{
			return mCustomFont;
		...
		set
		{
			if (mCustomFont != value)
			{
				...
				if (value != null)
				{
					SetActiveFont(value.trueTypeFont);
				}
				ProcessAndRequest();
				if (mActiveTTF != null)
					base.MarkAsChanged();
	...
	public UnityEngine.Object ambigiousFont
	{
		...
		{
			// EBG START
			if (mSelectorFont != null) return (UnityEngine.Object)(mSelectorFont);
			if (mCustomFont != null) return (UnityEngine.Object)(mCustomFont);
			// EBG END
		...
		set
		{
			...
			// EBG START
			FontSelector efs = value as FontSelector;
			CustomFontBase cf = value as CustomFontBase;
			if (cf != null)
			{
				customFont = cf;
				SetupCustomFont();
	...
	public string text
	{
		...
		{
			// EBG START
			if (Application.isPlaying && value != null && value.StartsWith("ID_"))
			{
				_id = value;
				value = EB.Localizer.GetString(value);
				
				if((mFont != null && mFont.onlyUppercase) || (mSelectorFont != null && mSelectorFont.selectedFont != null && mSelectorFont.selectedFont.onlyUppercase) || (mCustomFont != null && mCustomFont.isBitmap && mCustomFont.bitmapFont.onlyUppercase))
					value = value.ToUpper();
			}
			else
			{
...
#if DYNAMIC_FONT
	// EBG START
	bool isValid { get { return mFont != null || mTrueTypeFont != null || mSelectorFont != null || mCustomFont != null; } }
	// EBG END
#else
	...
	private void SetupCustomFont()
	{
		if (mCustomFont != null)
		{
			...
			UIFont activeFont = mCustomFont.bitmapFont;
			int activeFontSize = mCustomFont.bitmapFontSize;
			Font activeDynamicFont = mCustomFont.trueTypeFont;
			
			if (activeFont != null) mFontSize = activeFontSize;
			if (activeDynamicFont != null) SetActiveFont (activeDynamicFont);
		}
		else if (mSelectorFont != null)
		{
			SetActiveFont(null);
			
			UIFont activeFont = mSelectorFont.selectedFont;
			int activeFontSize = mSelectorFont.selectedFontSize;
			Font activeDynamicFont = mSelectorFont.selectedTrueTypeFont;
		...
		for (int i = 0; i < mList.size; ++i)
		{
			...
			if (lbl != null)
			{
				...
				if (fnt != null)
				{
					...
					lbl.MarkAsChanged();
					// EBG START - IF the panel it is on is static, give it a frame to update.
					// Otherwise the font text will be garbled.
					if (lbl.panel != null)
					{
						lbl.panel.singleFrameUpdate = true;
					}
					// EBG END
				}
			}
	...
	{
#if UNITY_EDITOR
		// EBG START - whatever for just in the editor?
//		if (!NGUITools.GetActive(this)) return;
	...
	bool mUsingTTF = true;
	// EBG START
	bool mUsingSelector = true;
	bool mUsingCustomFont = true;
	// EBG END
	...
	protected override void OnValidate ()
	{
		...
		if (NGUITools.GetActive(this))
		{
			...
			UIFont fnt = mFont;
			// EBG START
			FontSelector efs = mSelectorFont;
			CustomFontBase cf = mCustomFont;
			// EBG END

			// If the true type font was not used before, but now it is, clear the font reference
			// EBG START - if the selector font was not used before, but now it is, clear the ttf and font references
			if (!mUsingTTF && ttf != null)
			{
				fnt = null;
				efs = null;
				cf = null;
			}
			else if (mUsingTTF && fnt != null)
			{
				ttf = null;
				efs = null;
				cf = null;
			}
			else if (!mUsingSelector && efs != null)
			{
				ttf = null;
				fnt = null;
				cf = null;
			}
			else if (!mUsingCustomFont && cf != null)
			{
				ttf = null;
				fnt = null;
				efs = null;
	...
	{
		if (!isValid) return;
		// EBG START - Vastly reduce unnecessary ProcessText calls.
		if (!shouldBeProcessed) return;
		// EBG END - Vastly reduce unnecessary ProcessText calls.
		...
		// EBG START - Setting this true results in text being reprocessed every frame!
		// mChanged = true;
		// EBG END
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
...
#if DYNAMIC_FONT
					// EBG START - fix null reference exception
					NGUIText.fontScale = (isDynamic || mFont == null) ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;
					// EBG END
#else
				...
				// Wrap the text
				// EBG START - Use usage values
				float widthUsage, heightUsage;
				bool fits = NGUIText.WrapText(mText, out mProcessedText, out widthUsage, out heightUsage, true, overflowMethod == Overflow.ClampContent);

				if (mOverflow == Overflow.ShrinkContent && !fits)
				{
					// NOTE: KL - the math for this is not quite correct and is too aggressive in decreasing the font size
//					int oldPs = ps;
//					int widthPs = Mathf.RoundToInt((float)ps / widthUsage);
//					int heightPs = Mathf.RoundToInt((float)ps / heightUsage);
//					ps = Mathf.Min(widthPs, heightPs);
//					if (ps >= oldPs)
//					{
//						ps = oldPs - 1;
//					}
...
//					if (ps > 1) continue;
		...
		// EBG START - Store last width / height process values
		mLastWidth = mWidth;
		mLastHeight = mHeight;
		// EBG END - Store last width / height process values
	}
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		// EBG START - localization status colouring
		if (EB.Localizer.ShowStatuses && !string.IsNullOrEmpty(_id))
		{
			var old = col;
			col = EB.Localizer.GetColor(_id);
			col.a = old.a;
		}
		// EBG END
		...
		// EBG START - this stripped out symbol encoding for things like colouring...
		//string text = processedText;
		// EBG END
		...
		NGUIText.tint = col;
		// EBG START - why are we using bare text?
//		NGUIText.Print(text, verts, uvs, cols);
		NGUIText.Print(mProcessedText, verts, uvs, cols);
		// EBG END
	...
	public void UpdateNGUIText ()
	{
		...
		// EBG START - override with UIResolutionManager
		if (isDynamic && UIResolutionManager.Instance != null)
		{
			NGUIText.pixelDensity = UIResolutionManager.Instance.GetPixelSize();
			mDensity = NGUIText.pixelDensity;
		}
		else if (isDynamic && keepCrisp)
		{
			UIRoot rt = root;
