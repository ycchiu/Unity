...
public class UIPanel : UIRect
{
	...
	// EBG START - Flag to skip update if widgetsAreStatic is checked off 
	/// <summary>
	/// This panel will skip its update if widgetsAreStatic is checked.
	/// </summary>
	public bool skipUpdateIfWidgetsAreStatic = false;
	// EBG END

	/// EBG START - Single Frame Update allows panels to ignore the widgetsAreStatic flag for a single frame.
	public bool singleFrameUpdate = false;
	/// EBG END
	...
	// EBG START - SP - STORE VIEWSIZE HERE //
	...
	void Awake ()
	{
		...
		// EBG START - SP - SET THE VIEW SIZE ONCE AT START //
	...
	protected override void OnInit ()
	{
		...
		// EBG START - what does it help with? Do not add a rigid body.
		// EBG END
	...
	void UpdateTransformMatrix ()
	{
		...
		if (mMatrixFrame != fc)
		{
			...
			// EBG START - SP - Use viewSize instead of GetViewSize to speed it up
			Vector2 size = viewSize * 0.5f;
			// EBG END //
	...
	void UpdateSelf ()
	{
		// EBG START - SP - Don't update UIPanel if widgetsAreStatic flag is true, KL - and if skipUpdateIfWidgetsAreStatic is checked off
#if UNITY_EDITOR
		if (Application.isPlaying && widgetsAreStatic && skipUpdateIfWidgetsAreStatic && !singleFrameUpdate)
#else
		if (widgetsAreStatic && skipUpdateIfWidgetsAreStatic && !singleFrameUpdate)
#endif
		{
			return;
		}
		...
		for (int i = 0; i < widgets.size; ++i)
		{
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				if (mat != null || sdr != null || tex != null)
				{
					...
					// EBG START - uvs2 param added to call
					if (generateNormals) w.WriteToBuffers(dc.verts, dc.uvs, dc.uvs2, dc.cols, dc.norms, dc.tans);
					else w.WriteToBuffers(dc.verts, dc.uvs, dc.uvs2, dc.cols, null, null);
					// EBG END
				}
			}
		...
		if (dc != null)
		{
			...
			for (int i = 0; i < widgets.size; )
			{
				...
				if (w.drawCall == dc)
				{
					if (w.isVisible && w.hasVertices)
					{
						// EBG START - uvs2 param added to calls
						if (generateNormals) w.WriteToBuffers(dc.verts, dc.uvs, dc.uvs2, dc.cols, dc.norms, dc.tans);
						else w.WriteToBuffers(dc.verts, dc.uvs, dc.uvs2, dc.cols, null, null);
						// EBG END
					}
