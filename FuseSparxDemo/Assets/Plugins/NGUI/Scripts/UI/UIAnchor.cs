...
public class UIAnchor : MonoBehaviour
{
	...
	void Start ()
	{
		if (container == null && widgetContainer != null)
		{
			...
			// EBG START - cache widget/panel references
			mWidget = (container == null) ? null : container.GetComponent<UIWidget>();
			mPanel = (container == null && mWidget == null) ? null : container.GetComponent<UIPanel>();
			// EBG END
#if UNITY_EDITOR
		...
		// EBG START - look upwards in the hierarchy only
		if (uiCamera == null) uiCamera = EB.Util.FindComponentUpwards<Camera>(gameObject);
		// EBG END
...
//		UIPanel pc = mPanel;
		...
		{
			mTrans.position = v;
			// EBG START - Round to whole pixels
			if (runOnlyOnce)
			{
