...
public class UISprite : UIWidget
{
	...
	// EBG START - field is now exposed for property serialization
	public Flip flip { get { return mFlip; } set { mFlip = value; MarkAsChanged(); } }
	...
	override public int minWidth
	{
		get
		{
			if (type == Type.Sliced || type == Type.Advanced)
			{
				...
				UISpriteData sp = GetAtlasSprite();
				// EBG START - Apply atlas pixel size to padding.
				if (sp != null && atlas != null)
				{
					min += Mathf.RoundToInt((sp.paddingLeft + sp.paddingRight) * atlas.pixelSize);
				}
				// EBG END - Apply atlas pixel size to padding.
	...
	override public int minHeight
	{
		get
		{
			if (type == Type.Sliced || type == Type.Advanced)
			{
				...
				if (sp != null && atlas != null)
				{
					min += Mathf.RoundToInt((sp.paddingTop + sp.paddingBottom) * atlas.pixelSize);
				}
				// EBG END - Apply atlas pixel size to padding.
	...
	{
		// EBG START - Force invalidate and regenerate sprite.
		mSpriteSet = false;
		mSprite = GetAtlasSprite();
		// EBG END - Force invalidate and regenerate sprite.
		base.OnValidate();
...
#region Various fill functions
	...
	public override Vector4 drawingDimensions
	{
		get
		{
			...
			Vector4 br = border * (atlas != null ? atlas.pixelSize : 1f);
	...
	protected void TiledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		// EBG START - Effective division by zero will cause us to try to create an infinite number of verts.
