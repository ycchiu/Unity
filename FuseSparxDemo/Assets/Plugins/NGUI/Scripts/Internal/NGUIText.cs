...
static public class NGUIText
{
	...
	// EBG START
	static public Color ParseColor(string text, int offset, bool includeAlpha = false)
	// EBG END
	{
		...
		if (includeAlpha)
		{
			a = (NGUIMath.HexToDecimal(text[offset + 6]) << 4) | NGUIMath.HexToDecimal(text[offset + 7]);
		}
		// EBG END
		float f = 1f / 255f;
		// EBG START
	...
	// EBG START
	static public string EncodeColor(Color c, bool includeAlpha = false)
	// EBG END
	{
		// EBG START
		if (includeAlpha)
		{
			return NGUIMath.DecimalToHex(NGUIMath.ColorToInt(c), true);
		}
		// EBG END
		...
		ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike)
	{
		...
		// EBG START - increase length to account for 8-bit hex strings
		if (index + 10 > length) return false;
		// EBG END

		// EBG START - support for 8-bit hex strings that include alpha information
		if (text[index + 9] == ']')
		{
			Color c = ParseColor(text, index + 1, true);

			if (EncodeColor(c, true) != text.Substring(index + 1, 8).ToUpper())
				return false;
	...
	{
		// EBG START - new params
		float unused;
		return WrapText(text, out finalText, out unused, out unused, false, true);
		// EBG END - new params
	}
	...
	// EBG START - new usage params
	static public bool WrapText (string text, out string finalText, out float widthUsage, out float heightUsage, bool keepCharCount, bool clampHeight)
	// EBG END - new usage params
	{
		// EBG START - usage default vals.
		widthUsage = 0f;
		heightUsage = 0f;
		// EBG START - usage default vals.
		...
		// EBG START - Do the full text processing to find the required width / height 
		// unless clamp height is turned on.
		// NOTE: KL - disabling this condition for now
//		if (clampHeight)
		{
		...
		bool fits = true;
		bool eastern = false;
		// EBG START - Store the max required line width.
		int maxLineWidth = 0;
		// EBG END
		...
		for (; offset < textLength; ++offset)
		{
			...
			// EBG START - After building each line store the width that was used.
			maxLineWidth = Mathf.Max(maxLineWidth, Mathf.RoundToInt(rectWidth - remainingWidth));
			// EBG END
		...
		// EBG START - Now the text is processed, calculate usage for width & height
		widthUsage = (float)maxLineWidth / (float)rectWidth;
		heightUsage = (float)(lineCount * finalLineHeight) / (float)rectHeight;
		// NOTE: KL - default to previous fit calculations
//		if (heightUsage > 1f)
//		{
//			fits = false;
//		}
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		// EBG START - start with our TINT setting!
