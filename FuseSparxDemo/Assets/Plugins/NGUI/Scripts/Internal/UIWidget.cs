...
public class UIWidget : UIRect
{
	...
	// EBG START - cheap way of disabling rendering
	[HideInInspector][SerializeField] protected bool mIsRendering = true;
	// EBG END
	// EBG START - Enable / disable uvs2
	public virtual bool hasMultipleUVs { get { return false; } }
	// EBG END
	...
	// EBG START - A cheap way to disable a widget: ie. Don't disable the gameObject or component,
	//             which will cause a rebuild on the owner UIPanel, instead just don't render when
	//             this is turned off.
	public bool isRendering
	{
		set
		{
			if (mIsRendering != value)
			{
				mIsRendering = value;
				mChanged = true;
				// this covers the case where there was no geo or draw call for this widget, and we flip mIsRendering to true
				if (mIsRendering && (drawCall == null || geometry == null || !geometry.hasVertices))
				{
					Invalidate(true);
				}
			}
	...
	// EBG START - isRendering check
	public bool isVisible { get { return mIsRendering && mIsVisibleByPanel && mIsVisibleByAlpha && mIsInFront && finalAlpha > 0.001f && NGUITools.GetActive(this); } }
	...
	{
		// EBG START - mIsRendering check
		if (!mIsRendering || !mIsVisibleByAlpha || !mIsInFront)
		// EBG END
		{
			finalAlpha = 0f;
		}
	...
	public override void Invalidate (bool includeChildren)
	{
		...
		if (panel != null)
		{
			// EBG START - added panel.alwaysOnScreen check
			// panel.alwaysOnScreen
			bool vis;
			if (!hideIfOffScreen && !panel.clipsChildren || panel.alwaysOnScreen)
			{
				vis = true;
			}
			else
			{
				vis = panel.IsVisible(this);
			}
			// EBG END
	...
	public void CheckLayer ()
	{
		...
		{
			// EBG START - Supress this annoying warning. We cannot guarantee the layer will be the same as the panel it is under.
...
//				"If you want to move widgets to a different layer, parent them to a new panel instead.", this);
	...
	protected override void OnAnchor ()
	{
		...
		// EBG START - Whole numbers of pixels
	...
	// EBG START - virtual method
	protected virtual void OnDestroy ()
	{
		RemoveFromPanel();
	}
	// EBG END - virtual method
	...
	{
#if UNITY_EDITOR
		// EBG START - added singleFrameUpdate check
		if (!mMoved && !panel.widgetsAreStatic || !mPlayMode || panel.singleFrameUpdate)
#else
		if (!mMoved && !panel.widgetsAreStatic || panel.singleFrameUpdate)
		// EBG END - added singleFrameUpdate check
#endif
		{
	...
	public bool UpdateGeometry (int frame)
	{
		...
		// EBG START - mIsRendering check
		if (mIsRendering && mIsVisibleByAlpha && mLastAlpha != finalAlpha) mChanged = true;
		// EBG END
		...
		if (mChanged)
		{
			...
			// EBG START - mIsRendering check
			if (mIsRendering && mIsVisibleByAlpha && finalAlpha > 0.001f && shader != null)
			// EBG END
			{
				...
				{
					geometry.Clear();
					// EBG START - uvs2 implementation
					if (hasMultipleUVs)
					{
						OnFill(geometry.verts, geometry.uvs, geometry.uvs2, geometry.cols);
					}
					else
					{
	...
	// EBG START - adding uvs2 param
	public void WriteToBuffers (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Vector2> uvs2, BetterList<Color32> cols, BetterList<Vector3> normals, BetterList<Vector4> tangents)
	{
		geometry.WriteToBuffers(verts, uvs, uvs2, cols, normals, tangents);
	}
	// EBG END
	...
	// EBG START - adding second OnFill that supports uvs2 param
	virtual public void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Vector2> uvs2, BetterList<Color32> cols) { }
	// EBG END
}
