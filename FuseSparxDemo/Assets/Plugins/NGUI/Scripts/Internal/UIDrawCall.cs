...
public class UIDrawCall : MonoBehaviour
{
	...
	void CreateMaterial ()
	{
		...
		if (shader == null)
		{
			// If we still don't have a shader, use the default UI shader
			Debug.LogWarning("could not find any appropriate variant of shader " + shaderName);
			shader = Shader.Find("EBG/UI/Opaque");
		}
		// EBG END
	...
	public void UpdateGeometry ()
	{
		...
		// EBG START - Masking UVs are only supplied by widgets that are doing masking.
		// Pad them out if necessary:
		if (uvs2.size != 0)
		{
			int startSize = uvs2.size;
			int endSize = uvs.size;
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			if (verts.size < 65000)
			{
				...
				if (trim || verts.buffer.Length > 65000)
				{
					...
					// EBG START - Supply UVs2 if there are any.
					if (uvs2.size > 0)
					{
						mMesh.uv2 = uvs2.ToArray();
					}
					// EBG END
