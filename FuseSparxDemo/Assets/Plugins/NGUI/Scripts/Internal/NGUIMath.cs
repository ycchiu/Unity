...
static public class NGUIMath
{
	...
	[System.Diagnostics.DebuggerStepThrough]
	// EBG START
	static public string DecimalToHex (int num, bool use8Digits = false)
	// EBG END
	{
		// EBG START
		if (!use8Digits)
...
#else
		// EBG START
		if (use8Digits) return num.ToString ("X8");
		// EBG END
	...
	static public Bounds CalculateRelativeWidgetBounds (Transform trans, bool considerInactive, bool considerInvisible = true)
	// EBG END
	{
		// EBG START
		//return CalculateRelativeWidgetBounds(trans, trans, considerInactive);
		return CalculateRelativeWidgetBounds(trans, trans, considerInactive, considerInvisible);
		// EBG END
	...
	static public Bounds CalculateRelativeWidgetBounds (Transform relativeTo, Transform content, bool considerInactive, bool considerInvisible = true)
	// EBG END
	{
		if (content != null)
		{
			...
			if (widgets.Length > 0)
			{
				...
				for (int i = 0, imax = widgets.Length; i < imax; ++i)
				{
					...
					// EBG START
					if (!considerInvisible && !w.isVisible) continue;
					// EBG END
