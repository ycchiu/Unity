	...
	protected override bool ShouldDrawProperties ()
	{
		...
		if (NGUIEditorTools.DrawHeader("Advanced Options"))
		{
			...
			if (stat)
			{
				...
				GUILayout.BeginHorizontal();
				bool skipStatic = EditorGUILayout.Toggle("Skip Update", mPanel.skipUpdateIfWidgetsAreStatic, GUILayout.Width(100f));
				GUILayout.Label("Check if panel should not update when marked as static", GUILayout.MinWidth(20f));
				GUILayout.EndHorizontal();
				
				if (mPanel.skipUpdateIfWidgetsAreStatic != skipStatic)
				{
					mPanel.skipUpdateIfWidgetsAreStatic = skipStatic;
					mPanel.RebuildAllDrawCalls();
					EditorUtility.SetDirty(mPanel);
				}
			}
