...
public class NGUITransformInspector : Editor
{
	...
	private void RoundTinyFractionalValues(SerializedProperty prop, string propName)
	{
		var p = prop.FindPropertyRelative(propName);
		float f = p.floatValue;
		if (f != Mathf.Round(f) && EB.Util.FloatEquals(f, Mathf.Round(f), 0.001f))
		{
			p.floatValue = Mathf.Round(f);
		}
	}
	...
	void DrawPosition ()
	{
		GUILayout.BeginHorizontal();
		{
			...
			{
				// Round values to nearest integer if slightly off
				RoundTinyFractionalValues(mPos, "x");
				RoundTinyFractionalValues(mPos, "y");
				RoundTinyFractionalValues(mPos, "z");
			}
	...
	void DrawScale (bool isWidget)
	{
		GUILayout.BeginHorizontal();
		{
			...
			if (!Application.isPlaying)
			{
				RoundTinyFractionalValues(mScale, "x");
				RoundTinyFractionalValues(mScale, "y");
				RoundTinyFractionalValues(mScale, "z");
			}
