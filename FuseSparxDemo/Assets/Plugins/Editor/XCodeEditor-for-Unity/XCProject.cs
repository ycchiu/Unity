...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public XCProject( string filePath ) : this()
		{
			...
			string contents = File.ReadAllText(projectFileInfo.FullName);
		...
		public void ApplyMod( XCMod mod )
		{
			...
			foreach( string framework in mod.frameworks ) {
				...
				{
					completePath = System.IO.Path.Combine(mod.path, completePath);
				}
				else 
				{
		...
		public void Save()
		{
			...
			PBXParser parser = new PBXParser();
			using (	StreamWriter saveFile = File.CreateText( System.IO.Path.Combine( this.filePath, "project.pbxproj" ) ))
			{
