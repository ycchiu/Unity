...
- (NSString*) AdXGetBundleID {
    ...
    if (BundleID && [BundleID length] > 0)
        return [BundleID stringByAppendingFormat:@".iOS"];
...
#if defined(USEMACADDRESS) && USEMACADDRESS
- (NSString *) macAddress
{
...
- (void)doReportAppOpen
{
    ...
    if((!ierror) && ([(NSHTTPURLResponse *)response statusCode] == 200) ) {
        ...
        NSNotification *adxnot = [NSNotification notificationWithName:@"AdXDone" object:self];
        [[NSNotificationCenter defaultCenter] postNotification:adxnot];
    }
    [respStr release];
    [pool release];
...
extern "C" {
    ...
    void _reportAppOpen (const char* clientID, const char* iTunesID)
	{
	...
	void _SendEvent (const char* clientID, const char* eventname,const char* value, const char* currency)
	{
		if (trackerObject == nil)
			trackerObject = [[AdXTracking alloc] init];
		[trackerObject setClientId:AdXCreateNSString(clientID)];
